...
func (m *ServerMethods) MediaSeek(position int64) *dbus.Error {
	if err := m.mediaService.Seek(position); err != nil {
		return dbus.MakeFailedError(fmt.Errorf("failed to seek: %v", err))
	}
	return nil
}
